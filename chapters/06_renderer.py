...
class Renderer:
    def __init__(self, image_width: int, image_height: int, camera: Camera, bg_color: Optional[Color] = None) -> None:
    ...
    def render(self) -> None:
        for y in range(self._image.height):
            for x in range(self._image.width):
                color = self._get_pixel_color(x=x, y=y, gradient=True)
                self._image.add_pixel_color(x=x, y=y, color=color)

    def _get_pixel_color(self, x: int, y: int, gradient: bool) -> Color:
        if gradient:
            return Color(
                r=int(255 * (x / self._image_width)),
                g=int(255 * (y / self._image_height)),
                b=0,
            )
        else:
            return Color(
...
def main(width: int, height: int, output_path: Union[Path, str]) -> None:
    ...
    renderer = Renderer(image_width=width, image_height=height, camera=camera)
    renderer.render()
