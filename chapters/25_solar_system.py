...
def create_scene() -> Scene:
    # Materials
    sun_diffuse = Material(color=Color(255, 155, 0), random_color_range=(0.7, 1.0))
    sun_disperse = Material(color=Color(255, 100, 0), random_color_range=(0.7, 1.0), dispersion=0.85)
    merc_diffuse = Material(color=Color(169, 169, 169))
    venus_diffuse = Material(color=Color(225, 225, 224), random_color_range=(0.7, 1.0))
    earth_diffuse = Material(color=Color(0, 128, 255), random_color_range=(0.7, 1.0))

    mars_diffuse = Material(color=Color(204, 102, 0))
    jup_diffuse = Material(color=Color(189, 183, 107), random_color_range=(0.7, 1.0))
    sat_diffuse = Material(color=Color(238, 221, 130), random_color_range=(0.7, 1.0))
    urn_diffuse = Material(color=Color(173, 216, 230), random_color_range=(0.7, 1.0))
    nep_diffuse = Material(color=Color(70, 130, 180), random_color_range=(0.7, 1.0))

    # Planets
    sun = Sphere(center=Vec3(-6.7, 0, 5), radius=4, material=sun_diffuse)
    solar_winds = Sphere(center=Vec3(-6.7, 0, 5), radius=4.15, material=sun_disperse)
    mercury = Sphere(center=Vec3(-1.9 - 0.37, 0, 5), radius=0.07, material=merc_diffuse)
    venus = Sphere(center=Vec3(-1.57 - 0.37, 0, 5), radius=0.14, material=venus_diffuse)
    earth = Sphere(center=Vec3(-1.17 - 0.37, 0, 5), radius=0.15, material=earth_diffuse)
    mars = Sphere(center=Vec3(-0.84 - 0.37, 0, 5), radius=0.076, material=mars_diffuse)
    jupiter = Sphere(center=Vec3(-0.06 - 0.37, 0, 5), radius=0.6, material=jup_diffuse)
    saturn = Sphere(center=Vec3(1.18 - 0.37, 0, 5), radius=0.54, material=sat_diffuse)
    uranus = Sphere(center=Vec3(2.1 - 0.37, 0, 5), radius=0.3, material=urn_diffuse)
    neptune = Sphere(center=Vec3(2.72 - 0.37, 0, 5), radius=0.22, material=nep_diffuse)

    # Lights
    sun_light = PointLight(center=Vec3(-50, 0, 5), intensity=3)
    solar_wind_light = PointLight(center=Vec3(50, 0, 5), intensity=3, color=Color(255, 255, 255))
    rim = PointLight(center=Vec3(50, 30, 20), intensity=0.8)
    fill = PointLight(center=Vec3(40, -30, -30), intensity=0.4)
    sun_only_top = PointLight(center=Vec3(50, 350, 5), intensity=8)
    sun_only_bottom = PointLight(center=Vec3(50, -260, 5), intensity=5)

    # Light linking
    sun_only_top.add_include(sphere=sun)
    sun_only_top.add_include(sphere=solar_winds)
    sun_only_bottom.add_include(sphere=sun)
    sun_only_bottom.add_include(sphere=solar_winds)
    solar_wind_light.add_include(sphere=solar_winds)

    # Scene
    scene = Scene()
    scene.camera = Camera(center=Vec3(0, 0, -1), fov=0.26)

    scene.add_sphere(sphere=sun)
    scene.add_sphere(sphere=solar_winds)
    scene.add_sphere(sphere=mercury)
    scene.add_sphere(sphere=venus)
    scene.add_sphere(sphere=earth)
    scene.add_sphere(sphere=mars)
    scene.add_sphere(sphere=jupiter)
    scene.add_sphere(sphere=saturn)
    scene.add_sphere(sphere=uranus)
    scene.add_sphere(sphere=neptune)

    scene.add_light(light=sun_light)
    scene.add_light(light=rim)
    scene.add_light(light=fill)
    scene.add_light(light=sun_only_top)
    scene.add_light(light=sun_only_bottom)
...
def main() -> None:
    ...
    settings = RenderSettings(
        ...
        image_height=340,
        aa_samples=1.0,
        bg_color=Color(18, 8, 12),

        # Circular Noise Settings
        add_circular_noise=True,
        circular_noise_scale=0.8,
        circular_noise_color_min=200,
        circular_noise_color_max=255,
        circular_noise_size_min=0.002,
        circular_noise_size_max=0.008,
        ...
        tail_noise_size_min=0.001,
        tail_noise_size_max=0.003,
        tail_noise_width=50,
        ...
        pixel_noise_scale=0.06,
        pixel_noise_color_min=100,
        pixel_noise_color_max=255,
