...
class Renderer:
    def __init__(
            self, scene: Scene, render_settings: RenderSettings
    ) -> None:

        self._scene = scene

        aa_samples = render_settings.aa_samples

        if aa_samples < 1.0:
            raise RuntimeError(f'aa_samples={aa_samples} cannot be less that 1.0')

        self._aa_samples: float = aa_samples
        self._image_width: int = int(render_settings.image_width * self._aa_samples)
        self._image_height: int = int(render_settings.image_height * self._aa_samples)
        self._bg_color: Color = render_settings.bg_color
    ...
    def save_image(self, output_path: Union[Path, str]) -> None:
    ...
    def render(self) -> None:
        with progress_bar_context() as update_bar:
            for y in range(self._image.height):
                update_bar(percentage=(y / (self._image_height - 1)) * 100)
    ...
    def _raytrace(self, ray_dir: Vec3) -> Optional[Color]:
        ...
        if closest_sphere is not None:
            ...
            return self._get_pixel_color(sphere=closest_sphere, intersection_point=intersection)
        else:
            return None
    ...
    def _get_pixel_color(self, sphere: Sphere, intersection_point: Vec3) -> Optional[Color]:
...
def main(width: int, height: int, output_path: Union[Path, str]) -> None:
    ...
    renderer.render()
    renderer.save_image(output_path=str(output_path))
